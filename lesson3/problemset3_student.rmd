...
# add meaningful sorting variable for each country
data.order <- summarise(dplyr::group_by(data.tidy, country), mean(fraction))
colnames(data.order)[2] <- "mean.fraction"
data.joined <- dplyr::left_join(data.tidy, data.order, by="country")
dplyr::arrange(data.joined, desc(mean.fraction))

# Convert year to numeric
data.joined$year = as.numeric(levels(data.joined$year))[data.joined$year]

# Graph

# Graph "fraction"" as y and "country" re-ordered by mean.fraction as x
ggplot(data = data.joined, aes(x=reorder(country, -mean.fraction), y=fraction)) +
  ...
  geom_point(aes(y=mean.fraction, shape="O")) + 
  scale_shape_discrete(name="Mean", labels=c("Mean")) +
