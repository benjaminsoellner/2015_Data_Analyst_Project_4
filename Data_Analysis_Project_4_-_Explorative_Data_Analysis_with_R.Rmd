...
p1 <- ggplot_corr(
    ...
    xscale=c(3.5, 11, 0.5), yscale=c(2.6, 4.0, 0.4),
    xlab="Fixed acidity (tartaric acid) in g/l",
    ylab="pH",
...
p2 <- ggplot_corr(
    ...
    xscale=c(0, 0.9, 0.05), yscale=c(2.6, 4.0, 0.4),
    xlab="Volatile acidity (acetic acid) in g/l",
    ylab="pH",
...
p3 <- ggplot_corr(
    ...
    xscale=c(0, 1.4, 0.1), yscale=c(2.6, 4.0, 0.4),
...
p4 <- ggplot_corr(
    ...
    xscale=c(3.5, 12, 0.5), yscale=c(2.6, 4.0, 0.4),
...
However, most interesting is the corellation of the output variable
"quality" with any of the inputs. The corellation matrix shows, that
generally, more alcohol ($r = 0.45$), less total sulfur dioxide
($r = -0.19$), less salt ($r = -0.19$) and less volatile acidity
($r = -0.18$) leads to better wines.

```{r corellation table, warning=FALSE, message=FALSE, error=FALSE, echo=FALSE, fig.width=10, fig.height=7}
```

Factoring in domain knowledge about different wine flavors and,
conversely, different expectations on each flavor, we furthermore
come to the conclusion, that some of the properties are more
important for some wine flavors and less important for others.

As described in the graph below, chlorides have a larger negative influence
...
and total sulfur dioxide seems to decrease the quality of dry wines most
($r_\text{dry}(\text{total.sulfur.dioxide},\text{quality})=-0.26$).
Alcohol content, on the other hand, has the strongest positive impact
