...
```{r preparations, warning=FALSE, message=FALSE, echo=FALSE}
...
# gradient to show corellation with legend named "Corellation", transitioning
# from red to white to light yellow to blue mapped to -1.0,..,-0.1,0.1,..,1.0
cor_fill_gradient <- scale_fill_gradientn(name="Corellation",
                       colours=c(red_gradient,"lightyellow","lightyellow",blue_gradient),
...
# Function to add row to data frame at a specific point
insert_row_to_dataframe <- function(dataframe, newrow, rowindex) {
  dataframe <- rbind(dataframe, newrow)
  dataframe <- dataframe[order(c(1:(nrow(dataframe)-1),r-0.5)),]
  row.names(dataframe) <- 1:nrow(dataframe)
  return(dataframe)
}


# There is an issue with my version of ggcorr not displaying the corellation 
# values continuously: somehow it does not set the parameter "nbreaks"
# correctly. The following pulls the most up-to-date version from GitHub
# with which I do not encounter any problems (potential attack vector though).
...
# load data
setwd("h:/Dokumente/GitHub/DAND_4_ExplorativeDataAnalysisR/")
wine <- read.csv("project/wineQualityWhites.csv")
# read and display data structure
cat("Data set structure: ")
str(wine)
cat("Data set dimension: ")
dim(wine)
```
...
```{r fixed acitity, warning=FALSE, message=FALSE, echo=FALSE}
summary(wine$fixed.acidity)
cat("Standard deviation:", sd(wine$fixed.acidity))
...
cat("Standard deviation:", sd(wine$volatile.acidity))
...
```{r citric acidity, echo=FALSE}
summary(wine$citric.acid)
cat("Standard deviation:", sd(wine$citric.acid))
...
```{r pH, warning=FALSE, echo=FALSE}
summary(wine$pH)
cat("Standard deviation:", sd(wine$pH))
...
```{r chlorides, warning=FALSE, echo=FALSE}
summary(wine$chlorides)
cat("Standard deviation:", sd(wine$chlorides))
cat("75%-quantile:", quantile(wine$chlorides, 3/4))

ggplot_hist(
     data=wine, x="chlorides", binwidth=0.002, xscale=c(0, 0.2, 0.02),
...
```{r free sulfur dioxide, warning=FALSE, echo=FALSE}
summary(wine$free.sulfur.dioxide)
cat("Standard deviation:", sd(wine$free.sulfur.dioxide))
cat("The one outlier with free sulfur dioxide > 150 mg/l:")
...
```{r total sulfur dioxide, warning=FALSE, echo=FALSE}
summary(wine$total.sulfur.dioxide)
cat("Standard deviation:", sd(wine$total.sulfur.dioxide))

ggplot_normdist(
     data=wine, x="total.sulfur.dioxide", binwidth=2, xscale=c(0, 350, 20),
...
cat("Standard deviation:", sd(wine$sulphates))
...
cat("The only sweet wine with residual sugar > 45 g/l:")
...
```{r density, warning=FALSE, echo=FALSE}
summary(wine$density)
cat("Standard deviation:", sd(wine$density))
...
cat("Absolute frequency of output variable 'quality':")
table(wine$quality)
ggplot_hist(
...
The following graph shows the acidity distributions, each one with a different
color, all overlayed in one diagram. Notice how the distribution of citric 
acidity and volatile acidity is much skinnier and has a lower mean while
fixed acidity has a higher mean and broader distribution.
...
```{r volatile acidity + cidric acid + fixed acidity graph, warning=FALSE, echo=FALSE}
# ... then plot those as overlayed histograms
ggplot_hist(
     data=wineacids, x="acidvalue", binwidth=0.1, xscale=c(0, 11, 1),
...
are any two of the acidity values corellated? We can plot the corellation
matrix:

```{r relationship acids, warning=FALSE, echo=FALSE}
...
```{r relationship fixed acidity vs. cidric acid, warning=FALSE, echo=FALSE}
ggplot_corr(
...
```{r relationship volatile.acidity vs. acids, warning=FALSE, echo=FALSE}
p1 <- ggplot_corr(
    data=wine, x="volatile.acidity", y="fixed.acidity", 
    xscale=c(0, 0.9, 0.1), yscale=c(3.5, 11, 0.5), 
    xlab="Volatile acidity (acetic acid) in g/l",
...
p2 <- ggplot_corr(
    ...
    xscale=c(0, 0.9, 0.1), yscale=c(0, 1.4, 0.1),
    xlab="Volatile acidity (acetic acid) in g/l",
    ylab="Citric acidity in g/l",
...
acid components and drive the pH up. Displayed below are some corellation
matrices between pH and other variables as well as the respective
scatter plots relating both variables.

```{r relationship acidity vs. pH, warning=FALSE, echo=FALSE}
# Create aggregate (sum) variable of all acidities
wine$acidity <- wine$fixed.acidity + wine$volatile.acidity + wine$citric.acid

# Pearsons R
cat("Corellation matrix between fixed acidity and ...:")
...
cat("R^2 value between fixed acidity and ...:")
cor(wine[, c("fixed.acidity", "volatile.acidity", "citric.acid", "acidity")],
    wine$pH)^2

p1 <- ggplot_corr(
    data=wine, x="fixed.acidity", y="pH",
    xscale=c(3.5, 11, 1), yscale=c(2.6, 4.0, 0.4),
    xlab="Fixed acidity (tartaric acid) in g/l",
    ylab="pH",
...
p2 <- ggplot_corr(
    ...
    xscale=c(0, 0.9, 0.1), yscale=c(2.6, 4.0, 0.4),
    xlab="Volatile acidity (acetic acid) in g/l",
    ylab="pH",
...
p3 <- ggplot_corr(
    ...
    xscale=c(0, 1.4, 0.2), yscale=c(2.6, 4.0, 0.4),
...
p4 <- ggplot_corr(
    ...
    xscale=c(3.5, 12, 1), yscale=c(2.6, 4.0, 0.4),
...
```{r total vs. free sulfur dioxide, warning=FALSE, echo=FALSE}
cat("Correlation total vs. free sulfur dioxide:", 
...
```{r sulphates vs. total sulfur dioxide, warning=FALSE, echo=FALSE}
cat("Corellation sulphates vs. total sulfur dioxide:", 
    cor(wine$sulphates, wine$total.sulfur.dioxide))
cat("R^2 value sulphates vs. total sulfur dioxide:", 
    cor(wine$sulphates, wine$total.sulfur.dioxide)^2)
cat("Corellation sulphates vs. free sulfur dioxide:", 
...
```{r residual sugar vs. alcohol, echo=FALSE, warning=FALSE, error=FALSE}
cat("Corellation between residual sugar and alcohol content: ",
    cor(wine$residual.sugar, wine$alcohol))

ggplot_corr(
    data=wine, x="residual.sugar", y="alcohol", 
    xscale=c(0, 32, 2), yscale=c(8, 14, 0.5),
    xlab="Residual sugar in g/dm³", 
...
```{r density vs. residual sugar vs. alcohol, warning=F, fig.width=15, fig.height=10, error=FALSE, echo=FALSE}
cat("Corellation wine density vs. ...")
...
p2 <- ggplot_corr(
    ...
    xscale=c(0, 32, 2), yscale=c(0.985, 1.005, 0.005),
    xlab="Residual sugar in g/dm³",
    ylab="Density in g/cm³",
...
```{r flavor distribution, echo=FALSE}
# Show how many wines have each flavor
cat("Distribution of wine flavors:")
table(wine$flavor)
```
...
```{r flavor scatterplot, echo=FALSE}
...
```{r flavor histogram, echo=FALSE}
ggplot_hist(
     data=wine, x="residual.sugar", binwidth=0.2, xscale=c(0, 32, 2),
     fill="flavor", fillname="Wine Flavor",
     title="Distribution of residual sugar (non-sweet wines)",
     xlab="Residual sugar in g/dm³",
     ylab="Number of wines with this residual sugar content"
  ) +
  scale_fill_brewer(name="Flavor", type="qual", palette=6)
```
...
```{r scatterplot matrix, fig.width=15, fig.height=20, warning=FALSE, message=FALSE, error=FALSE, echo=FALSE}
set.seed(1836)
ggpairs(data=wine[sample.int(nrow(wine), 1000), c(2:14)],
        title="Relationship between any two variables",
        alpha=0.05)
```

The following corellation table also shows the corellation values between
any two variables again and highlights visually highest and lowest
corellation values:

```{r corellation table, warning=FALSE, message=FALSE, error=FALSE, echo=FALSE, fig.width=10, fig.height=7}
# plot a corellation table with all the numerical values and use the 
# gradient we defined above
ggcorr(data=wine[sample.int(nrow(wine), 1000), c(2:14)], name="r",
       label=TRUE, hjust=1, angle=-45, size=3, label_round=2) + 
  cor_fill_gradient + 
...
```{r r for acidity vs. all types of acidity, echo=FALSE, echo=FALSE}
cat("Corellation of acidity with...:")
...
```{r r for pH vs. all types of acidity, echo=FALSE}
cat("Corellation of pH with...:")
...
```{r r for total.sulfur.dioxide vs. free.sulfur.dioxide, echo=FALSE}
cat("Corellation of total sulfur dioxide with free sulfur dioxide:")
cor(wine$total.sulfur.dioxide, wine$free.sulfur.dioxide)
```
**Strong positive corellation** because free sulfur dioxide makes out the
majority of all sulfur dioxides.

```{r r for sulphates vs. total.sulfur.dioxide, echo=FALSE}
cat("Corellation of sulphates with total sulfur dioxide:", 
    cor(wine$sulphates, wine$total.sulfur.dioxide))
```
**Weak positive corellation**, as the description of sulphates suggests,
sulphates lead to the creation of SO2 levels.

```{r r for alcohol vs. residual.sugar, echo=FALSE}
cat("Corellation of alcohol content with residual sugar:", 
...
```{r r for residual.sugar vs. alcohol, echo=FALSE}
cat("Corellation of alcohol content with residual sugar:", 
...
```{r r for free.sulfur.dioxide vs. total.sulfur.dioxide, echo=FALSE}
cat("Corellation of free sulfur dioxide with total sulfur dioxide:", 
...
```{r r for residual.sugar vs. free.sulfur.dioxide, echo=FALSE}
cat("Corellation of residual sugar with free sulfur dioxide:", 
...
```{r r for chlorides vs. alcohol, echo=FALSE}
cat("Corellation of chlorides with alcohol content:", 
...
```{r r for chlorides and total.sulfur.dioxide vs. density, echo=FALSE}
cat("Corellation of density with ...:")
...
```{r r for acidity vs. density, echo=FALSE}
cat("Corellation of acidity with density:", 
    cor(wine$acidity, wine$density))
```
**Weak positive corellation**, acidity further increases the density of wine.

```{r r for alcohol vs. density, echo=FALSE}
cat("Corellation of alcohol content with density:", 
...
```{r r for quality, echo=FALSE}
cat("Corellation of quality with...:")
...
```{r scatterplot matrix by quality, warning=FALSE, fig.width=15, fig.height=20, message=FALSE, error=FALSE, echo=FALSE}
set.seed(1836)
winematrix <- wine
winematrix$quality <- as.factor(winematrix$quality)
ggpairs(data=winematrix[sample.int(nrow(winematrix), 1000), c(2:14)],
...
play a higher or lower role. The first line ("any (summary)" shows the 
general corellation again, as was displayed in the previous plot.

Blue signifies positive corellation (positive
...
```{r r for quality by wine - dataframe, warning=FALSE}
...
```{r r for quality by wine - graph, warning=FALSE, fig.width=10, fig.height=7, echo=FALSE}
# Plot a tile graph with the different corellation indicators horizontally
# and the wine flavors vertically arranged
ggplot(winequality, aes(x=indicator, y=flavor)) +
  geom_tile(aes(fill=corellation)) +
  # use corellation gradient
  cor_fill_gradient + 
...
```{r chemical composition of different wine flavors, warning=FALSE, fig.width=15, fig.height=10, echo=FALSE}
...
We that corellation and linear fit are not everything: for example,
...
```{r chemical composition of different wine flavors and influence on quality, warning=FALSE, fig.width=15, fig.height=10, echo=FALSE}
...
```{r chemical composition of different wine flavors and influence on quality 2, warning=FALSE, message=FALSE, fig.width=15, fig.height=10, echo=FALSE}
...
* **Volatile acidity** should not be too low (bad wines,
scores 3-4) and not too high (average wines, scores 5-7). Half-dry
...
In this paper 4898 wines were compared relating 11 of their chemical
properties with an expert rating of the wine's quality. The wines were
grouped into different wine flavors (classical-dry, dry, half-dry, semi-sweet
and sweet) according to their residual sugar content and 'total' acidity
(referring to the sum of all types of acidity concentrations: fixed, volatile
and citric). The below graph shows all the wines combined with their
classification (color), sugar content (x axis) and acidity (y axis). Notice,
how generally, the acidity levels are spread out more for dryer wines.

```{r flavor scatterplot, echo=FALSE}
```

The corellation of different wine properties with each other
were thoroughly analyzed. Some obvious chemical relations were
identified, like the negative corellation between sugar content
and alcohol or the pH value or (positive) corellation between
pH and any of the acidity values.

However, the corellations of different chemical properties with
wine quality are most interesting. The following plot shows 
corellation of some of those properties with wine quality in the
fist line as well as a break-down of those corellation 
by wine flavor in the remaining lines.

We see that most influential for good quality are alcohol content
($r(\text{alcohol},\text{quality}) = 0.45$), and that bad wine 
quality is mostly due high total sulfur dioxide levels
($r(\text{total.sulfur.dioxide},\text{quality}) = -0.19$),
salt levels ($r(\text{chlorides},\text{quality}) = -0.19$)
and volatile acidity levels
($r(\text{volatile.acidity},\text{quality}) = -0.18$).

Taking into consideration the break-down by flavor, we find:

* Chlorides have a larger negative influence
the more sweeter the wine
($r_\text{semi-sweet}(\text{chlorides},\text{quality})=-0.297$)
* Volatile acidity seems to have a particular bad impact on half-dry wines
($r_\text{half-dry}(\text{volatile.acidity},\text{quality})=-0.307$)
* Total sulfur dioxide seems to decrease the quality of dry wines most
($r_\text{dry}(\text{total.sulfur.dioxide},\text{quality})=-0.26$)
* Alcohol content has the strongest positive impact
for the driest wines
($r_\text{classical-dry}(\text{alcohol},\text{quality})=0.458$,
$r_\text{semi-sweet}(\text{alcohol},\text{quality})=0.16$).

```{r r for quality by wine - graph, warning=FALSE, fig.width=10, fig.height=7, echo=FALSE}
```

However, (linear) corellation values are not everything:
looking at the mean (and standard-error) of each wine
flavor scored with a particulary score, we can definetely
see some surprising non-linear trends for some of the
chemical properties. See belows graph. 

The most surprising finding here is that for some of the
chemical properties, there seems to be an actual "sweet
spot" - more of an ideal value of said distribution than
an actual linear trend. This applies to:

* salt content for all wines except half-dry wines
* sulfur dioxide for some medium-range dry or half-dry wines
* volatile acidity for all (classical)-dry wines: should not
be too low (bad wines scored 3-4) and not too high (average
wines scored 5-7).

In addition to that, citric acidity seems to follow an 'uncanny
valeys' where a certain low citrus contant scores the wine
worse and only for a quite high citrus contant, the wine
is perceived as excellent.
